...
$variables = $this->balance_model->cargar_balances('2013');
...
<?php if(count($variables) > 0){ ?>
	<script type="text/javascript">
	    ...
	    $(document).ready(function(){
        	...
        	$('#select_balance' + id_variable + ' > option[value="<?php echo $variable->id_balance; ?>"]').attr('selected', 'selected');
	    	...
	    	if(id_variable_comparacion > 0){
	    		 ...
	    		 */
                variables = ajax("<?php echo site_url('balance/cargar'); ?>", {'tipo': 'V', 'anio': $("#select_balance" + id_variable + " option:selected").text(), 'id_variable': id_dimension}, "JSON");
	    		 ...
	    		 */
                dimensiones = ajax("<?php echo site_url('balance/cargar'); ?>", {'tipo': 'D', 'anio': $("#select_balance" + id_variable + " option:selected").text(), 'id_variable': id_categoria}, "JSON");
		    	 ...
		    	 */
		    	categorias = ajax("<?php echo site_url('balance/cargar'); ?>", {'tipo': 'C', 'anio': $("#select_balance" + id_variable + " option:selected").text(), 'id_variable': null}, "JSON");
		         ...
		         */
	        	$('#select_variable' + id_variable + ' > option[value="<?php echo $variable->id_variable_comparacion; ?>"]').attr('selected', 'selected');
    ...
    $(document).ready(function(){
        ...
        if (balances.length != 0) {
			...
			$.each(balances, function(key, val){
	            //Se agrega cada categoría al select
	            balance.append("<option value='" + val.ano + "'>" + val.ano + " - " + val.strNombre + "</option>");
	        });//Fin each
        }else{
    	...
    	balance.on("change", function(){
    		...
    		if($(this).val() != ""){
    			//Cargaremos las categorías
                categorias = ajax("<?php echo site_url('balance/cargar'); ?>", {'tipo': 'C', 'anio': $(this).val(), 'id_variable': null}, "JSON");
    	...
    	categoria.on("change", function(){
    		...
    		if($(this).val() != ""){
    			//Cargaremos las dimensiones
                dimensiones = ajax("<?php echo site_url('balance/cargar'); ?>", {'tipo': 'D', 'anio': $("#select_balance" + id_variable + " option:selected").text(), 'id_variable': $(this).val()}, "JSON");
    	...
    	dimension.on("change", function(){
    		...
    		if($(this).val() != ""){
    			//Cargaremos las variables
                variables = ajax("<?php echo site_url('balance/cargar'); ?>", {'tipo': 'V', 'anio': $("#select_balance" + id_variable + " option:selected").text(), 'id_variable': $(this).val()}, "JSON");
    	...
    	$("#btn_guardar_variable" + id_variable).on("click", function(){
            ...
            if(actualizar == 'true'){
    	        ...
    	        mostrar_mensaje('Registro exitoso', 'La variable se actualizó exitosamente.');
            }
    	});
    });
</script>
...
<?php //if(false){ ?>
	<script type="text/javascript">
	    ...
	    $(document).ready(function(){
        	...
        	$('#select_categoria' + id_variable + ' > option[value="<?php echo $variable->fuente; ?>"]').attr('selected', 'selected');
