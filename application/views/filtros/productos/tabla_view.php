...
<script type="text/javascript">
    ...
    function actualizar_filtro(elemento, titulo){
    	// imprimir("actualizando filtro")
        //Se pone el título del filtro
        $("#titulo_filtro").html('').text(titulo);
        
        // //Cargamos la tabla
        $("#cont_filtros").load("listas/cargar_interfaz", {tipo: 'tabla_filtros_productos'});
    ...
    function CargarCondiciones(elemento){
    	// imprimir("Cargando condicionales" + total_condidionales);
        $("#cont_condicionales").html('').append("<div id='condicional" + total_condidionales + "'></div>");
        // //alert(elemento);
        condiciones = ajax("<?php echo site_url('filtros/cargar_filtros'); ?>", {"tipo": 'condiciones_producto', 'codigo_filtro': elemento}, "JSON");                
        ...
        if(condiciones.length > 0){
            $.each(condiciones, function(key, val){
                ...
                $("#condicional" + total_condidionales).load("<?php echo site_url('filtros/agregar/condicional_producto'); ?>", {numero: total_condidionales, datos:condicionest });

                // Se alimenta el valor
